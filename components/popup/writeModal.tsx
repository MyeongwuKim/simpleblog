...
export default function WriteModal({
  ...
  const extractTumbnail = () => {
    ...
    if (process.env.NEXT_PUBLIC_DEMO) {
      ...
      thumbnail = demoThumb ? demoThumb[1] : null;
    } else {
      // ✅ 실제: Cloudflare 이미지
      const realThumb = writeThumbnail?.match(
        /imagedelivery\.net\/[^\/]+\/([^\/]+)/
      );
      thumbnail = realThumb ? realThumb[1] : null;
    }
    return thumbnail;
  };
  ...
  return (
    <Modal
      id="alert-modal"
      applyTheme={{
        header: { close: "replace", base: "replace" },
        content: {
          inner: "merge",
        },
        root: {
      ...
      <ModalBody>
        ...
        >
          ...
          >
            ...
            <p className="line-clamp-1 text-text2 leading-[1.5em]">{preview}</p>
          </div>
        </Card>
      </ModalBody>
      <ModalFooter>
